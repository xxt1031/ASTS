...
from src.calibloss import \
    ...
    NodewiswClassECE
from src.utils_EdgeConcat_v3_rdplot import \
    set_global_seeds, arg_parse, name_model, create_nested_defaultdict, \
    metric_mean, metric_std, default_cal_wdecay, save_prediction, plot_reliabilities
...
def main(dataset_index, split, init, eval_type_list, args):
    ...
    # uncal_reliability_fold_conf =torch.zeros(15).to(device) 
    # uncal_reliability_fold_acc=torch.zeros(15).to(device)
    # uncal_reliability_fold_count=torch.zeros(15, dtype=torch.long).to(device)
    uncal_reliability_fold = [torch.zeros(15).to(device) , torch.zeros(15).to(device),torch.zeros(15, dtype=torch.long).to(device)]
    # cal_reliability_fold_conf=torch.zeros(15).to(device)
    # cal_reliability_fold_acc=torch.zeros(15).to(device)
    # cal_reliability_fold_count=torch.zeros(15, dtype=torch.long).to(device)
    cal_reliability_fold = [torch.zeros(15).to(device) , torch.zeros(15).to(device),torch.zeros(15, dtype=torch.long).to(device)]
    # uncal_reliability_fold = Reliability(torch.zeros(15), torch.zeros(15), ctorch.zeros(15))
    # cal_reliability_fold = Reliability(torch.zeros(15), torch.zeros(15), ctorch.zeros(15))
    for fold in range(max_fold):
        ...
        for eval_type in eval_type_list:
            ...
            for i in range(3):
                uncal_reliability_fold[i] += reliability[i]
        
            # plot_reliabilities(reliability, args, uncal=True)
        ...
        for eval_type in eval_type_list:
            eval_result, reliability = eval(data, log_prob, 'Train')
            for metric in eval_result:
        ...
        for eval_type in eval_type_list:
            ...
            for i in range(3):
                cal_reliability_fold[i] += reliability[i]
        
            # plot_reliabilities(reliability, args, uncal=False)
            ...
            for metric in eval_result:
                ...
                #     print('nll_temp:', nll_temp)
        torch.cuda.empty_cache()
    return uncal_test_result, cal_val_result, cal_test_result, uncal_reliability_fold, cal_reliability_fold
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    uncal_reliability_total = [torch.zeros(15).to(device) , torch.zeros(15).to(device),torch.zeros(15, dtype=torch.long).to(device)]
    cal_reliability_total = [torch.zeros(15).to(device) , torch.zeros(15).to(device),torch.zeros(15, dtype=torch.long).to(device)]
        ...
        for split in range(max_splits):
            for init in range(max_init):
                ...
                (uncal_test_result,
                 cal_val_result,
                 cal_test_result, 
                 uncal_reliability_fold, cal_reliability_fold) = main(int(dataset_index+1), split, init, eval_type_list, args)
                # print(f'uncal_test_nll: {uncal_test_result[eval_type]['nll']}')
                # print(f'cal_test_kde: {cal_test_result[eval_type]['nll']}')
                for i in range(3):
                    uncal_reliability_total[i] += uncal_reliability_fold[i]
                    cal_reliability_total[i] += cal_reliability_fold[i]
    ...
    print('uncal_reliability_total', uncal_reliability_total)
    print('cal_reliability_total', cal_reliability_total)
    plot_reliabilities(uncal_reliability_total, args, uncal=True)
    plot_reliabilities(cal_reliability_total, args, uncal=False)
