...
from src.data.data_utils import load_data,load_syn_data, load_node_to_nearest_training, load_node_to_nearest_training_4syndata
from src.model.model import create_model
from src.calibrator.calibrator_config import \
    TS, VS, ETS, CaGCN, GATS, IRM, SplineCalib, Dirichlet, OrderInvariantCalib, GAETS
...
def main(dataset_index, split, init, eval_type_list, args):
    ...
    for fold in range(max_fold):
        ...
        for eval_type in eval_type_list:
            ...
            # plot_title = f'Uncal_{model_name}_{args.dataset}_{split}_{init}'
            # figure_saveto = Path(os.path.join('figure','reliability_diagram', str(args.dataset)))
            # # figure_saveto.mkdir(parents=True, exist_ok=True)
            # plot_reliabilities(reliability , plot_title, figure_saveto)
        ...
        for eval_type in eval_type_list:
            ...
            # plot_title = f'calibrated_{args.calibration}_{model_name}_{args.dataset}_{split}_{init}'
            # figure_saveto = Path(os.path.join('figure','reliability_diagram', str(args.dataset)))
            # plot_reliabilities(reliability, plot_title, figure_saveto)
            for metric in eval_result:
