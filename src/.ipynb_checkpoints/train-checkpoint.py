...
from src.data.data_utils import load_data, load_syn_data


# os.environ['CUDA_LAUNCH_BLOCKING'] = "1" #xxt
...
def main(dataset_index, split, init, args):
    # Evaluation
    ...
    for fold in range(max_fold):
        ...
        if args.dataset in ['syn-cora','syn-products']:
            data = load_syn_data(args.dataset, args.homoEdgeRatio, dataset_index, args.split_type, split, fold)
        else:
        ...
        if args.dataset in ['syn-cora','syn-products']:
            ...
            dir = Path(os.path.join('model', args.dataset, str(raw_file_name), args.split_type, 'split'+str(split), 
                                    'init'+ str(init)))
        else:
...
if __name__ == '__main__':
    ...
    for dataset_index in range(num_datasets):
        for split in range(max_splits):
            for init in range(max_init):
                val_result, test_result = main(int(dataset_index+1), split, init, args)
    ...
    print("Val  Accuracy" + " " * 5 + "Val  NLL" +  " " * 5 + "Test Accuracy" + " " * 5 +  "Test NLL")
    print(f"{val_mean['acc']:.2f}\t" + f"{val_mean['nll']:.4f}\t" + f"{test_mean['acc']:.4f}\t" + f"{test_mean['nll']:.4f}") 
    # print("Val  Accuracy" + " " * 5 + "Val  NLL" +  " " * 5 + "Val ECE" + " " * 5 + "Test Accuracy" + " " * 5 +  "Test NLL" + " "*5 + "Test ECE")
    # print(f"{val_mean['acc']:.2f}\t" + f"{val_mean['nll']:.4f}\t" + f"{val_mean['ece']:.4f}\t" + f"{test_mean['acc']:.4f}\t" + f"{test_mean['nll']:.4f}\t" + f"{test_mean['ece']:.4f}") 