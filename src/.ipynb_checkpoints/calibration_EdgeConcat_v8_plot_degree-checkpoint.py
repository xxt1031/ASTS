...
from src.data.data_utils import load_data,load_syn_data, load_node_to_nearest_training, \
    load_node_to_nearest_training_4syndata, compute_node_level_homophily, compute_degree_bins
...
def main(dataset_index, split, init, eval_type_list, args):
    ...
    uncal_group_eces_temp = torch.zeros(20)
    group_eces_temp = torch.zeros(20)
    for fold in range(max_fold):
        ...
        groups = compute_degree_bins(data.edge_index)
        # print(groups)
        uncal_group_eces = torch.zeros(20)
        group_eces = torch.zeros(20)
        ...
        uncal_group_eces_temp += uncal_group_eces
        group_eces_temp += group_eces
        
        torch.cuda.empty_cache()
    
    return uncal_test_result, cal_val_result, cal_test_result, uncal_group_eces_temp/3, group_eces_temp/3
...
if __name__ == '__main__':
    ...
    uncal_group_eces_total = torch.zeros(20)
    group_eces_total = torch.zeros(20)
        ...
        for split in range(max_splits):
            for init in range(max_init):
                ...
                (uncal_test_result,
                 cal_val_result,
                 cal_test_result, 
                 uncal_group_eces, group_eces) = main(int(dataset_index+1), split, init, eval_type_list, args)
                # print(f'uncal_test_nll: {uncal_test_result[eval_type]['nll']}')
                # print(f'cal_test_kde: {cal_test_result[eval_type]['nll']}')
                uncal_group_eces_total += uncal_group_eces
                group_eces_total += group_eces
    ...
    bin_centers = np.arange(0.025, 1, 0.05)
    ...
    plt.bar(bin_centers - bar_width, uncal_group_eces_total.numpy()/25, bar_width*2, align='center', label='Uncalibrated', color=uncal_color, alpha=0.6)
    plt.bar(bin_centers + bar_width, group_eces_total.numpy()/25, bar_width *2, align='center', label=f'{args.calibration}', color=cal_color, alpha=0.6)
    ...
    plt.xticks(x_axis, [f'{x:.2f}' for x in x_axis], rotation=45)

    # Saving plots
    figure_filename = f'ece_degree_{args.model}_{args.calibration}_{args.dataset}_mean.png'
    figure_path = Path(os.path.join('figure', 'ece_results', 'degree'))
