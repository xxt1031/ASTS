...
from src.calibrator.CaEC_layer_v8_test30 import CaEC_Layer
...
def fit_calibration(temp_model, eval, data, train_mask, test_mask, patience = 100):
    ...
    for epoch in range(2000):
        ...
        calibrated = eval(logits, flag=True)
        ...
        with torch.no_grad():
            temp_model.eval()
            calibrated = eval(logits, flag=False)
...
class ETS(nn.Module):
    ...
    def fit(self, data, train_mask, test_mask, wdecay, lr):
        self.to(device)
        self.temp_model.fit(data, train_mask, test_mask, wdecay, lr)
...
class Dirichlet(nn.Module):
    ...
    def fit(self, data, train_mask, test_mask, wdecay, lr, patience=100):
        self.to(device)
        optimizer = optim.Adam(self.dir.parameters(), lr=lr)
        vlss_mn = float('Inf')
...
class OrderInvariantCalib(nn.Module):
    ...
    def calibrate(self, logits, flag=False):
    ...
    def fit(self, data, train_mask, test_mask, wdecay, lr):
        ...
        self.optimizer = optim.Adam(
            self.train_param, lr=lr, weight_decay=wdecay)
        fit_calibration(
...
class CaEC(nn.Module):
    def __init__(self, model, edge_index, num_nodes, num_class, hidden_channels_CaEC):
        super().__init__()
        self.model = model
        self.num_nodes = num_nodes
        self.caec =  CaEC_Layer(in_channels=num_class,
                                 hidden_channels = hidden_channels_CaEC,
    ...
    def graph_temperature_scale(self, logits, flag=False):
        """
        Perform graph temperature scaling on logits
        """
        temperature = self.caec(logits, flag).view(self.num_nodes, -1)
        return temperature.expand(self.num_nodes, logits.size(1))

   
    def fit(self, data, train_mask, test_mask, wdecay, lr):
        self.to(device)
        def eval(logits, flag):
            temperature = self.graph_temperature_scale(logits, flag)
            calibrated = logits / temperature
            return calibrated

        self.train_param = self.caec.parameters()
        self.optimizer = optim.Adam(self.train_param, lr=lr, weight_decay=wdecay)
        fit_calibration(self, eval, data, train_mask, test_mask)
        return self