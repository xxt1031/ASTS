...
from torch_geometric.utils import remove_self_loops, add_self_loops, softmax, degree, is_undirected, dropout_edge
...
class CaEC_Layer(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], flag=False):
        ...
        if flag:
            edge_index_dropout, _ = dropout_edge(self.edge_index, p=0.5)
        ...
        if flag:
            ...
            out = self.propagate(edge_index=edge_index_dropout, h=x*deg_inverse.unsqueeze(-1))
        else:
            if self.undirected:
                deg = self.degrees
            else:
