...
# from src.calibrator.my_algo import New_CalibAttentionLayer
...
class ETS(nn.Module):
    ...
    def fit(self, data, train_mask, test_mask, wdecay):
        ...
        self.w1.data = torch.tensor(w[0], dtype=torch.float32)
        self.w2.data = torch.tensor(w[1], dtype=torch.float32)
        self.w3.data = torch.tensor(w[2], dtype=torch.float32)
    ...
    def ensemble_scaling(self, logit, label, t):
        ...
        # p0 = np.exp(logit)/(np.sum(np.exp(logit), axis=1)[:, None] + 1e-12)
        max_logit = np.max(logit, axis=1, keepdims=True)
        exps = np.exp(logit - max_logit)
        p0 = exps / (np.sum(exps, axis=1, keepdims=True) + 1e-12)[:,None]
...
class CaGCN(nn.Module):
    ...
    def graph_temperature_scale(self, logits, edge_index):
        ...
        """
        new_data = Data(x=logits, edge_index=edge_index)
        temperature = self.cagcn(new_data)
        return temperature
...
class GATS(nn.Module):
    ...
    def forward(self, data):
        ...
        # print('logits / temperature:', logits / temperature)
        return logits / temperature
    ...
    def fit(self, data, train_mask, test_mask, wdecay, lr):
        self.to(device)
        def eval(logits):
            temperature = self.graph_temperature_scale(logits)
            calibrated = logits / temperature
        ...
        self.optimizer = optim.Adam(self.train_param, lr=lr, weight_decay=wdecay)
        fit_calibration(self, eval, data, train_mask, test_mask)
        return self
...
class IRM(nn.Module):
    ...
    def forward(self, data):
        ...
        # return torch.log(torch.from_numpy(p_calib).to(device) + torch.finfo().tiny)
        return torch.from_numpy(p_calib).to(device) + torch.finfo().tiny
...
class Dirichlet(nn.Module):
    ...
    def forward(self, data):
        return self.calibrate(self.model(data))
    ...
    def fit(self, data, train_mask, test_mask, wdecay, patience=100):
        self.to(device)
...
class SplineCalib(nn.Module):
    ...
    def forward(self, data):
        ...
        temp_score = (min(scores)>=0 and max(scores)<=1)
...
# Incorprate ETS ideas into the coding framework of the Temperature computations of GATS
class GAETS(nn.Module):
    def __init__(self, model, edge_index, num_nodes, train_mask, num_class, dist_to_train, gats_args):
        super().__init__()
        self.model = model
        self.num_nodes = num_nodes
        self.cagat = New_CalibAttentionLayer(in_channels=num_class,
                                         out_channels=1,
                                         edge_index=edge_index,
                                         num_nodes=num_nodes,
                                         num_class= num_class,
    ...
    def fit(self, data, train_mask, test_mask, wdecay):
        ...
        fit_calibration(self, eval, data, train_mask, test_mask)
        return self
# Incorprate ETS ideas into the coding framework of GATS      
# class GAETS(nn.Module):
#     def __init__(self, model, edge_index, num_nodes, train_mask, num_classes, dist_to_train, gats_args):
#         super().__init__()
#         self.model = model
#         self.num_nodes = num_nodes
#         self.num_classes = num_classes
#         self.cagat = CalibAttentionLayer(in_channels=num_classes,
#                                          out_channels=1,
#                                          edge_index=edge_index,
#                                          num_nodes=num_nodes,
#                                          train_mask=train_mask,
...
#                                          heads=gats_args.heads,
#                                          bias=gats_args.bias)
#         self.w_raw = Parameter(torch.ones(3))

#     def forward(self, data):
#         logits = self.model(data)
#         temperature = self.graph_temperature_scale(logits)
#         w = F.softmax(self.w_raw, dim=0)
#         p = w[0] * F.softmax(logits / temperature, dim=1) + w[1] * F.softmax(logits, dim=1) + w[2] * 1/self.num_classes
#         # print('logits/temperature:', logits/temperature)
#         # print('p:', torch.log(p))
#         # return torch.log(p)
#         return p
...
#             temperature = self.graph_temperature_scale(logits)
#             w = F.softmax(self.w_raw, dim=0)
#             calibrated = w[0] * F.softmax(logits / temperature, dim=1) + w[1] * F.softmax(logits, dim=1) + w[2] * 1/self.num_classes
#             return calibrated

#         # self.train_param = self.cagat.parameters()
#         self.train_param = list(self.cagat.parameters()) + [self.w_raw]
#         self.optimizer = optim.Adam(self.train_param, lr=0.01, weight_decay=wdecay)
#         fit_calibration(self, eval, data, train_mask, test_mask)
#         return self

# Incorprate GATS codes into the coding framework of ETS
# class GAETS(nn.Module):
#     def __init__(self, model, edge_index, num_nodes, train_mask, num_classes, dist_to_train, gats_args):
#         super().__init__()
#         self.model = model
...
#         self.w3 = nn.Parameter(torch.zeros(1))
#         self.num_classes = num_classes
#         self.temp_model = GATS(model, edge_index, num_nodes, train_mask,
#                             num_classes, dist_to_train, gats_args)

#     def forward(self, data):
#         logits = self.model(data)
#         temp = self.temp_model(data)
#         p = self.w1 * F.softmax(logits / temp, dim=1) + self.w2 * F.softmax(logits, dim=1) + self.w3 * 1/self.num_classes
#         output = torch.log(p)
#         output[output == -float('inf')] = -1e3
#         return output
...
#         label = data.y[train_mask]
#         # label = data.y
#         one_hot = torch.zeros_like(logits[train_mask])
#         one_hot.scatter_(1, label.unsqueeze(-1), 1)
#         # print('logits.shape', logits.shape)
#         # print('train_mask', torch.sum(train_mask))
#         # print('test_mask', torch.sum(test_mask))
#         temp = self.temp_model.graph_temperature_scale(logits)
#         w = self.ensemble_scaling(logits[train_mask].cpu().detach().numpy(), one_hot.cpu().detach().numpy(), temp[train_mask].cpu().detach().numpy())
#         # self.w1, self.w2, self.w3 = w[0], w[1], w[2]
#         self.w1.data = torch.tensor(w[0], dtype=torch.float32)
#         self.w2.data = torch.tensor(w[1], dtype=torch.float32)
#         self.w3.data = torch.tensor(w[2], dtype=torch.float32)
