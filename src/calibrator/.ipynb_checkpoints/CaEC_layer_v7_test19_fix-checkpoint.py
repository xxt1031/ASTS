...
class CaEC_Layer(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], flag=False):
        ...
        if flag:
            ...
            out = self.propagate(edge_index=edge_index_dropout, h=h*deg_inverse_sqrt.unsqueeze(-1), probs=probs, logprobs=logprobs, conf=conf)
            ...
            out = self.propagate(edge_index=self.edge_index, h=h*deg_inverse_sqrt.unsqueeze(-1), probs=probs, logprobs=logprobs, conf=conf)
        ...
        #     nan_indices = torch.where(torch.isnan(sim))[0]
        #     # sortedconfs, sortindices = torch.sort(nodeconfs)
        #     # print('nodelogits:', nodelogits[nan_indices])
        #     print('sim:', sim[nan_indices])

        # if torch.isnan(dconf).any():
        #     nan_indices = torch.where(torch.isnan(dconf))[0]
        #     # sortedconfs, sortindices = torch.sort(nodeconfs)
        #     # print('nodelogits:', nodelogits[nan_indices])
        #     print('sim:', dconf[nan_indices])
            
        out = self.mlp(sim, input_tensor=True)
        # print('out.shape:', out.shape)
        ...
        #     # sortedconfs, sortindices = torch.sort(nodeconfs)
        #     # print('nodelogits:', nodelogits[nan_indices])
        #     print('logits_in_layer_before_softplus:', out[nan_indices])
        out = F.softplus(out +  self.conf_coef * dconf * deg_inverse.unsqueeze(-1)) + F.softplus(self.bias)
        # if torch.isnan(out).any():
        #     nan_indices = torch.where(torch.isnan(out))[0]
        #     # sortedconfs, sortindices = torch.sort(nodeconfs)
        #     # print('nodelogits:', nodelogits[nan_indices])
        #     print('logits_in_layer:', out[nan_indices])
        return out
    ...
    def message(
        ...
        if torch.isnan(weights).any():
            nan_indices = torch.where(torch.isnan(weights))[0]
            # sortedconfs, sortindices = torch.sort(nodeconfs)
            # print('nodelogits:', nodelogits[nan_indices])
            print('weights:', weights[nan_indices])
        weights = softmax(weights, index, ptr, size_i)
        
        return torch.cat([
            h_j * weights.unsqueeze(-1).expand_as(h_j),
