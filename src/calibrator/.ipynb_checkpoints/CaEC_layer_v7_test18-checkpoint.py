...
class CaEC_Layer(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], flag=False):
        if flag:
            ...
            # two_hop_edge_index = self.init_adj(edge_index_dropout)
        ...
        if flag:
            ...
            out = self.propagate(edge_index=edge_index_dropout, h=h*deg_inverse.unsqueeze(-1), probs=probs, logprobs=logprobs)
            # out2 = self.propagate(edge_index=two_hop_edge_index, h=h*deg_inverse_2.unsqueeze(-1), probs=probs, logprobs=logprobs)
        else:
            if self.undirected:
                deg = self.degrees
            else:
            ...
            out = self.propagate(edge_index=self.edge_index, h=h*deg_inverse.unsqueeze(-1), probs=probs, logprobs=logprobs)
