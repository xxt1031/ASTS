...
class GAETS(nn.Module):
    def __init__(self, model, edge_index, num_nodes, train_mask, num_class, dist_to_train, gats_args):
        ...
        self.model = model
        self.num_nodes = num_nodes
        self.dir = nn.Linear(num_class, num_class)
...
class CaEC(nn.Module):
    ...
    def forward(self, data):
        logits = self.model(data)
        temperature = self.graph_temperature_scale(logits)
        return self.calibrate(logits / temperature)
    ...
    def fit(self, data, train_mask, test_mask, wdecay, lr, patience=100):
        self.to(device)
        self.train_param = list(self.caec.parameters()) + list(self.dir.parameters())
        optimizer = optim.Adam(self.train_param, lr=lr)
        vlss_mn = float('Inf')
        ...
        for epoch in range(2000):
            ...
            optimizer.zero_grad()
            logits = self.model(data)
            temperature = self.graph_temperature_scale(logits, flag=True)
            calibrated = self.calibrate(logits / temperature)
            loss = F.cross_entropy(
                calibrated[train_mask], labels[train_mask]
            ) + self.odir_loss(wdecay, wdecay)
            loss.backward()
            ...
            with torch.no_grad():
                self.eval()
                temperature = self.graph_temperature_scale(logits, flag=False)
                calibrated = self.calibrate(logits / temperature)
                val_loss = F.cross_entropy(
                    calibrated[test_mask], labels[test_mask]
                ) + self.odir_loss(wdecay, wdecay)
