...
class ECE(nn.Module):
    ...
    def __init__(self, bins: int = 15, scheme: str = 'equal_width', norm=1):
        """
...
class NodewiseECE(NodewiseMetric):
    ...
    def forward(self, logits: Tensor, gts: LongTensor) -> Tensor:
        ...
        nodeconfs, nodepreds = torch.softmax(nodelogits, -1).max(dim=-1)
        nodecorrects = (nodepreds == nodegts)
        # if torch.isnan(nodeconfs).any():
        #     nan_indices = torch.where(torch.isnan(nodeconfs))[0]
        #     # sortedconfs, sortindices = torch.sort(nodeconfs)
        #     print('nodelogits:', nodelogits[nan_indices])
        #     print('nodeconfs:', nodeconfs[nan_indices])
...
class KDE: 
    ...
    def ece_kde(confidence, correct, norm=1, kbw_choice='correct'):
        ...
        # print(f'correct_conf: {correct_conf}, type: {type(correct_conf)}')
        # print("correct is : ", correct, (correct==1))
        if correct_conf.size == 0:
           kde_value = torch.tensor(0, dtype=torch.float64)
        else:
            N = confidence.size(0)
            ...
            # print(f'kbw: {kbw}, confidence: {confidence}')
            if kbw <= 0 or torch.all(confidence == 1.0):
                kde_value = torch.tensor(0, dtype=torch.float64)
                ...
                kde_value = torch.trapz(integral[ind],x_int[ind]) / torch.trapz(pp2[ind],x_int[ind])
                # print(kde_value)
        return kde_value